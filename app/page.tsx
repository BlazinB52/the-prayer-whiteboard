...
async function getFeaturedHomepageData(): Promise<FeaturedHomepageData | null> {
  ...
  return { teaching: selectedTeaching as FeaturedHomepageData["teaching"], highlights, chalkboard };
}
...
function FeaturedHomepage({ data }: { data: FeaturedHomepageData }) {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f7f2e8] text-[#243126]">
      ...
      <StaticHomepageLowerSections excludeFeatured />
    </main>
  );
}

function StaticHomepageLowerSections({ excludeFeatured = false }: { excludeFeatured?: boolean }) {
  const gatherings = excludeFeatured ? archiveDates.filter((gathering) => !gathering.current) : archiveDates;
  return <><section className="px-5 py-14 sm:px-8 sm:py-20"><div className="mx-auto grid max-w-6xl gap-5 md:grid-cols-2"><article id="prayer" className="rounded-[2rem] border border-[#284a3b]/10 bg-[#fffdf8] p-6 shadow-xl shadow-[#4d5f52]/8 sm:p-8"><span className="grid size-12 place-items-center rounded-2xl bg-[#e8efe9] text-[#244a3a]"><HeartHandshake aria-hidden="true" size={25} /></span><p className="mt-6 text-xs font-extrabold uppercase tracking-[0.18em] text-[#8e673c]">Stand together</p><h2 className="mt-2 text-3xl font-extrabold tracking-tight text-[#243d31]">Prayer Needs</h2><p className="mt-4 leading-7 text-[#5b6a61]">Join us in praying for salvation, healing, families, our communities, Israel, and the nations. Prayer needs are shared only in summary form, with names and identifying details withheld to protect privacy.</p><span className="mt-6 inline-flex items-center gap-2 font-extrabold text-[#9d5a2f]">Current requests coming soon <ArrowRight aria-hidden="true" size={18} /></span></article><article id="praise" className="rounded-[2rem] border border-[#d2a34f]/20 bg-[#fff8e8] p-6 shadow-xl shadow-[#8b6531]/8 sm:p-8"><span className="grid size-12 place-items-center rounded-2xl bg-[#f5dfaa] text-[#8a5425]"><MessageCircleHeart aria-hidden="true" size={25} /></span><p className="mt-6 text-xs font-extrabold uppercase tracking-[0.18em] text-[#8e673c]">Remember His goodness</p><h2 className="mt-2 text-3xl font-extrabold tracking-tight text-[#503b27]">Praise Reports</h2><p className="mt-4 leading-7 text-[#6f604f]">Celebrate answered prayer, testimonies of God&apos;s faithfulness, and the ways we see Him working throughout the week.</p><span className="mt-6 inline-flex items-center gap-2 font-extrabold text-[#9d5a2f]">Praise reports coming soon <ArrowRight aria-hidden="true" size={18} /></span></article></div></section><section id="gatherings" className="border-y border-[#284a3b]/10 bg-[#eee7da] px-5 py-14 sm:px-8 sm:py-20"><div className="mx-auto max-w-6xl"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#946332]">Return to the Word</p><h2 className="mt-2 text-4xl font-extrabold tracking-tight text-[#243d31]">Previous Gatherings</h2></div><p className="max-w-md text-sm leading-6 text-[#607066]">Teachings will be organized by gathering date so they remain easy to find later.</p></div><div className="mt-8 grid gap-4 md:grid-cols-3">{gatherings.map((gathering) => <article key={gathering.date} className="group flex min-h-[245px] flex-col rounded-3xl border border-[#284a3b]/10 bg-[#fffdf8] p-6"><div className="flex items-center justify-between gap-3"><span className="inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.12em] text-[#9b6531]"><CalendarDays aria-hidden="true" size={16} /> {gathering.date}</span>{gathering.current ? <span className="rounded-full bg-[#e7efe9] px-3 py-1 text-[10px] font-black uppercase tracking-wider text-[#326048]">Latest</span> : null}</div><h3 className="mt-5 text-xl font-extrabold leading-7 text-[#263e33]">{gathering.title}</h3><p className="mt-3 text-sm leading-6 text-[#66746c]">{gathering.description}</p><span className="mt-auto inline-flex items-center gap-2 pt-6 text-sm font-extrabold text-[#9d5a2f]">Teaching coming soon <ArrowRight aria-hidden="true" size={17} /></span></article>)}</div></div></section><footer className="bg-[#1d352b] px-5 py-10 text-center text-[#d8e5dd] sm:px-8"><BookOpenText aria-hidden="true" className="mx-auto text-[#efc775]" size={28} /><p className="mt-4 text-lg font-extrabold text-white">The Whiteboard</p><p className="mt-2 text-sm">Prayer · The Word · Praise · Growing Together</p></footer></>;
}
