...
export default async function PublicDevotionalDayPage({ params }: { params: Promise<{ slug: string; dayNumber: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#f7f2e8] text-[#243126]">
      <PublicHeader maxWidthClassName="max-w-3xl" end={<Link href={`/devotionals/${slug}`} className="shrink-0 text-sm font-extrabold text-[#244a3a]">Devotional overview</Link>} />
      <article className="mx-auto max-w-3xl px-5 py-10 sm:px-8 sm:py-16">
        <header className="border-b border-[#284a3b]/15 pb-8">
          ...
          <p className="mt-4 text-sm font-bold text-[#607066]">{series.title} for {series.teaching.title}</p>
        </header>
        ...
        <nav className="mt-12 flex flex-col gap-3 border-t border-[#284a3b]/15 pt-6 sm:flex-row sm:items-center sm:justify-between">
          {dayNumber > 1 ? <Link href={`/devotionals/${slug}/day/${dayNumber - 1}`} className="inline-flex items-center gap-2 font-extrabold text-[#9d5a2f]"><ArrowLeft aria-hidden="true" size={18} /> Previous day</Link> : <span />}
          {dayNumber < 7 ? <Link href={`/devotionals/${slug}/day/${dayNumber + 1}`} className="inline-flex items-center gap-2 font-extrabold text-[#9d5a2f]">Next day <ArrowRight aria-hidden="true" size={18} /></Link> : null}
        </nav>
        <div className="mt-6 flex flex-wrap gap-4 text-sm font-extrabold text-[#244a3a]">
          <Link href={`/devotionals/${slug}`}>Return to devotional overview</Link>
          <Link href={`/teachings/${series.teaching.slug}`}>Return to full teaching</Link>
        </div>
      </article>
      <PublicFooter />
      <ReturnToTop />
